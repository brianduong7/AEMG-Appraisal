...
/**
 * Top-right notification control: managers and HR see KPI / appraisal submissions.
 */
export function HeaderNotificationsButton() {
  ...
  const canReceive = recipientId != null;
    ...
    <div className="relative shrink-0" ref={wrapRef}>
      <button
        ...
        onClick={() => {
          setOpen((o) => {
            const next = !o;
            if (next && canReceive) void refresh();
            return next;
          });
        }}
        className="relative flex h-9 w-9 items-center justify-center rounded-full border border-white/20 bg-white/10 text-white shadow-sm transition hover:border-gold-400/60 hover:bg-white/20"
        aria-label={
          canReceive
        ...
        >
          ...
          {!canReceive && (
            <p className="px-3 py-4 text-sm text-zinc-600">
              Sign in as a manager or HR to see KPI submissions and review
              updates.
            </p>
          )}
          {canReceive && count === 0 && (
                ...
                <li key={n.id}>
                  ...
                  >
                    ...
                    <span className="block text-xs text-zinc-500">
                      {mode === "hr"
                        ? "Submitted KPIs · Open appraisal"
                        : "Submitted KPIs · Open review"}
                    </span>
                  </Link>
                </li>
              ))}
