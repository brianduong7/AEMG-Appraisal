...
export function HomeContent() {
  ...
  /** Appraisal awaiting delete confirmation - null means the dialog is closed. */
  ...
  async function deleteAppraisalById(id: string) {
    setDeleteBusy(true);
    setDeleteError(null);
    try {
    ...
    <AppShell active="list">
                      ...
                      <td className="px-5 py-3.5 text-right">
                        {caps.canSuperAdmin ? (
                          <div className="flex items-center justify-end gap-2">
                            ...
                            <button
                              type="button"
                              className="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-200 bg-white text-red-600 shadow-sm transition hover:border-red-300 hover:bg-red-50"
                              title="Delete"
                              aria-label="Delete"
                              onClick={(e) => {
                                e.stopPropagation();
      ...
      {deleteTarget && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="delete-appraisal-title"
        >
          <div className="w-full max-w-sm rounded-xl bg-white p-5 shadow-xl">
            <h2
              id="delete-appraisal-title"
            ...
            <p className="mt-2 text-sm text-slate-600">
              <strong>
                {appraisalListDisplayName(deleteTarget, "hr", user)}
              </strong>
              &apos;s {erpAppraisalCycleLabel(deleteTarget.cycleYear)}{" "}
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button
                type="button"
                disabled={deleteBusy}
                onClick={() => void deleteAppraisalById(deleteTarget.id)}
                className="rounded-lg bg-red-600 px-3.5 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
              >
